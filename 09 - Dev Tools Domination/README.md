...
这个部分主要是实践了一些在 Console 面板里的调试技巧。按F12打开console面板进行调试

## 过程指南

### 第一步：封装调试

将调试的函数进行封装，不会直接暴露，有利于我们的调试
...
```javascript
    let console = {
      ...
      log(...args){
        ...
        window.console.log("%c停下！","font-szie:60x; color:red");
...
console.log是最常用的，但它还有一些更多功能：比如参数支持类似 C 语言的字符串替换模式。
...
前面是判断条件，后面是条件错误会显示的结果（接受一个表达式作为参数）。
...
### 表格形式输出
 ...
 `console.table()` 方法可以将数组、对象以表格的形式打印输出，还可以进行简单的筛选，例如如果只输出其中的某一列，可以加上第二个参数，示例如下。
...
//输出结果  fetching data: 780.489013671875ms
````
