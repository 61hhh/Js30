...
在输入框中输入一个字母，迅速匹配，查找包含该字母的城市并展示相关信息

初始文档中提供了 HTML 和 CSS，我们需要补全 JS 代码。不过fetch的json文件由于是在外网上的，常常会出现`Failed to load resource: net::ERR_CONNECTION_TIMED_OUT`的错误。
...
1. 声明一个空数组cities[]，用于存放解析 json 后的数据
...
其实本实验使用XMLHttpRequest()和fetch()效果一样。
...
````js
    ...
    fetch(endpoint)
		...
		.then(data => poetrys.push(...data));
     	//...data相当于对data解构
      	// .then(data => console.log(...data));//使用consol.log可以查看数据
		//用let cities时，可以data => (cities=data),与上面的等价
````
...
**注：**关于正则表达式的部分，比较复杂，因此直接copy了作者的code